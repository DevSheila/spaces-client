...
export const Header = () => {
  ...
  const linkClases = (type = null) => {
    ...
    if (type === subpage) {
      classes += ' active   text-green-800 font-semibold '; //active
    } else {
      classes += ' hover:text-green-800'; //inactive
    }
    return classes;
  };
      ...
      >
        ...
        >
          <a href="/" className="flex items-center gap-1">
            <img className="h-8 w-8 md:h-10 md:w-10" src="/logo.svg" alt="" />

            <span className=" text-2xl font-bold text-green-800 ">SPACES</span>
          </a>
          ...
          <Link
            ...
            className="w-50 flex h-full items-center gap-2 rounded-full  px-1 py-1 "
          >
              ...
              {user ? (
                <Avatar>
                  {user?.picture ? (
                    ...
                    <AvatarImage
                      src={`https://avatar.iran.liara.run/username?username=${user.name.replace(/\s+/g, '+')}`}
